...
public final class StubTransactionManager
  ...
  private <T, E extends Exception> T withTransaction(ThrowingTransactionalSupplier<T, E> work)
      throws E {
    Connection mockConnection = Utils.createProxy(Connection.class, (method, args) -> {
      log.info("Called mock Connection.{}({})", method.getName(),
          args == null
              ? ""
              : Arrays.stream(args)
    ...
    try (ThreadLocalTransaction transaction = pushTransaction(new ThreadLocalTransaction(mockConnection))) {
