 ...
 * <a href="https://microservices.io/patterns/data/transactional-outbox.html">Transactional
 * Outbox</a> pattern for Java. See
 * <a href="https://github.com/gruelbox/transaction-outbox">README</a> for usage instructions.
 */
public interface TransactionOutbox
{
   ...
   /**
    * Performs initial setup, making the instance usable. If
    * {@link TransactionOutboxBuilder#initializeImmediately(boolean)} is true, which is the default,
    * this method is called automatically when the instance is constructed.
    */
   void initialize();
    ...
    * the <em>real</em> method to occur after the current transaction is committed (as such a
    * transaction needs to be active and accessible from the transaction manager supplied to
    * {@link TransactionOutboxBuilder#transactionManager(TransactionManager)}),
    * <p>
    * Usage:
    *
    * <pre>
    ...
    * </pre>
    * <p>
    * This will write a record to the database using the supplied {@link Persistor} and
    * {@link Instantiator}, using the current database transaction, which will get rolled back if
    * the rest of the transaction is, and thus never processed. However, if the transaction is
    * committed, the real method will be called immediately afterwards using the submitter supplied
    * to {@link TransactionOutboxBuilder#submitter(Submitter)}. Should that fail, the call will be
    * reattempted whenever {@link #flush()} is called, provided at least supplied
    * {@link TransactionOutboxBuilder#attemptFrequency(Duration)} has passed since the time the task
    * was last attempted.
   ...
   /**
    * Starts building a schedule request with parameterization. See
    * {@link ParameterizedScheduleBuilder#schedule(Class)} for more information.
    * @return Builder.
    */
    ...
    * tried less than {@link TransactionOutboxBuilder#blockAfterAttempts(int)} )} times) and
    * attempts to resubmit them.
    * <p>
    ...
    * the scheduled work takes and how large {@link TransactionOutboxBuilder#flushBatchSize(int)}
    * is.
    * <p>
    * Calls {@link TransactionManager#inTransactionReturns(TransactionalSupplier)} to start a new
    * transaction for the fetch.
    * <p>
    * Additionally, expires any records completed prior to the
    * {@link TransactionOutboxBuilder#retentionThreshold(Duration)}.
   ...
   abstract class TransactionOutboxBuilder
   {
       ...
       *           commit and roll back transactions as well as interact with running transactions
       *           started outside.
       * @return Builder.
       */
       ...
       *           that class at runtime from the name. See {@link Instantiator} for more
       *           information. Defaults to {@link Instantiator#usingReflection()}.
       * @return Builder.
       */
      ...
      /**
       * @param submitter Used for scheduling background work. If no submitter is specified,
       *           {@link TransactionOutbox} will use {@link Submitter#withDefaultExecutor()}. See
       *           {@link Submitter#withExecutor(Executor)} for more information on designing
       *           bespoke submitters for remoting.
       * @return Builder.
       */
      ...
      /**
       * @param attemptFrequency How often tasks should be re-attempted. This should be balanced
       *           with {@link #flushBatchSize} and the frequency with which {@link #flush()} is
       *           called to achieve optimum throughput. Defaults to 2 minutes.
       * @return Builder.
       */
      ...
      /**
       * @param blockAfterAttempts how many attempts a task should be retried before it is
       *           permanently blocked. Defaults to 5.
       * @return Builder.
       */
       ...
       *           balanced with {@link #attemptFrequency} and the frequency with which
       *           {@link #flush()} is called to achieve optimum throughput. Defaults to 4096.
       * @return Builder.
       */
      ...
      /**
       * @param clockProvider The {@link Clock} source. Generally best left alone except when
       *           testing. Defaults to the system clock.
       * @return Builder.
       */
       ...
       *           This encapsulates all {@link TransactionOutbox} interaction with the database
       *           outside transaction management (which is handled by the
       *           {@link TransactionManager}). Defaults to a multi-platform SQL implementation that
       *           should not need to be changed in most cases. If re-implementing this interface,
       *           read the documentation on {@link Persistor} carefully.
       * @return Builder.
       */
       ...
       *           Includes a full stack trace. Defaults to {@code WARN} level, but you may wish to
       *           reduce it to a lower level if you consider warnings to be incidents.
       * @return Builder.
       */
       ...
       *           Context) in serialized invocations and recreate the state in submitted tasks.
       *           Defaults to true.
       * @return Builder.
       */
      ...
      /**
       * @param retentionThreshold The length of time that any request with a unique client id will
       *           be remembered, such that if the same request is repeated within the threshold
       *           period, {@link AlreadyScheduledException} will be thrown.
       * @return Builder.
       */
       ...
       *           automatically on creation (this is the default). Set to false in environments
       *           where structured startup means that the database should not be accessed until
       *           later.
       * @return Builder.
       */
   ...
   interface ParameterizedScheduleBuilder
   {
       ...
       * cause the request to be retained in the database after completion for the specified
       * {@link TransactionOutboxBuilder#retentionThreshold(Duration)}, during which time any
       * duplicate requests to schedule the same request id will throw
       * {@link AlreadyScheduledException}. This allows tasks to be scheduled idempotently even if
       * the request itself is not idempotent (e.g. from a message queue listener, which can usually
       * only work reliably on an "at least once" basis).
       * @param uniqueRequestId The unique request id. May be {@code null}, but if non-null may be a
       *           maximum of 250 characters in length. It is advised that if these ids are
       *           client-supplied, they be prepended with some sort of context identifier to ensure
       *           global uniqueness.
       * @return Builder.
       */
