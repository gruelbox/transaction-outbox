...
public class DefaultPersistor implements Persistor {
  ...
  private static final String ALL_FIELDS =
      "id, uniqueRequestId, invocation, nextAttemptTime, attempts, failed, processed, version";
  ...
  private void setupInsert(
    ...
    stmt.setBoolean(6, entry.isFailed());
  ...
  @Override
  public boolean retryable(Transaction tx, String entryId) throws Exception {
    PreparedStatement stmt =
        tx.prepareBatchStatement(
            "UPDATE "
                + tableName
                + " SET attempts = 0, failed = false "
                + "WHERE failed = true AND processed = false AND id = ?");
        ...
        tx.connection()
            .prepareStatement(
                ...
                "SELECT "
                    ...
                    + " FROM "
                    + tableName
                    + " WHERE nextAttemptTime < ? AND failed = false AND processed = false LIMIT ?"
                    + forUpdate)) {
