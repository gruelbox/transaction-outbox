...
final class OracleDialect extends Dialect {
  ...
  public Stream<SqlMigration> migrations(String tableName) {
    return Stream.of(
        ...
        new SqlMigration(
            8, "Update length of invocation column on outbox for MySQL dialects only.", "SELECT * FROM dual"),
        new SqlMigration(
            9,
        ...
        new SqlMigration(
            ...
            String.format(
                "UPDATE %s SET blocked = 0 WHERE blocked IS NULL",
                tableName)),
        new SqlMigration(
                11,
                "Fix data types on blocked and processed columns - part 2",
                String.format("UPDATE %s SET processed = 0 WHERE processed IS NULL",
                        tableName)),
        new SqlMigration(
                12,
                "Fix data types on blocked and processed columns - part 3",
                String.format("ALTER TABLE %s MODIFY processed NUMBER(1) NOT NULL",
                        tableName)),
        new SqlMigration(
                13,
                "Fix data types on blocked and processed columns - part 4",
                String.format("ALTER TABLE %s MODIFY blocked NUMBER(1) NOT NULL",
                        tableName))
    );
  }
  ...
  @Override
  public CompletableFuture<Integer> createVersionTableIfNotExists(Function<String, CompletableFuture<Integer>> statementInvoker) {
    return statementInvoker.apply("CREATE TABLE TXNO_VERSION AS SELECT CAST(0 AS NUMBER) as version FROM DUAL")
            .exceptionally(t -> {
