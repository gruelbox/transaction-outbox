...
public enum Dialect
{
   MY_SQL_5(false, Constants.DEFAULT_DELETE_EXPIRED_STMT), MY_SQL_8(true, Constants.DEFAULT_DELETE_EXPIRED_STMT), POSTGRESQL_9(true,
      "DELETE FROM {{table}} WHERE ctid IN (SELECT ctid FROM {{table}} WHERE nextAttemptTime < ? AND processed = true AND blocked = false LIMIT ?)"), ORACLE(true,
         "DELETE FROM {{table}} WHERE ctid IN (SELECT ctid FROM {{table}} WHERE nextAttemptTime < ? AND processed = true AND blocked = false AND ROWNUM <= ?)"), H2(false,
            Constants.DEFAULT_DELETE_EXPIRED_STMT);

   /**
    * @return True if hot row support ({@code SKIP LOCKED}) is available, increasing performance
    *         when there are multiple instances of the application potentially competing to process
    *         the same task.
    */
