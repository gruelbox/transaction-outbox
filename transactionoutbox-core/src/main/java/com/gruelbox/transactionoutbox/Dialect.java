...
public enum Dialect {
  ...
  POSTGRESQL_9(
      true,
      "DELETE FROM {{table}} WHERE ctid IN (SELECT ctid FROM {{table}} WHERE nextAttemptTime < ? AND processed = true AND failed = false LIMIT ?)"),
  ...
  private static class Constants {
    static final String DEFAULT_DELETE_EXPIRED_STMT =
        "DELETE FROM {{table}} WHERE nextAttemptTime < ? AND processed = true AND failed = false LIMIT ?";
  }
}
